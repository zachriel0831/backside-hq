 ...
 * */
var fpRead = function () {
    g_cus.CommonConnect(window.location.origin + "/api/News/NewsRead", { type: 'msg1' }, function (res) {
...
var cInsert = function () {
    ...
    Ueditor();  // 初始化編輯器
    $('#input').append(
        $('<p>', { 'html': '類型:' }).append($('<select>', { 'id': 'input_type', 'readonly': 'readonly', 'value': 'msg1' })),
        $('<p>', { 'html': '主題區名稱:' }).append($('<select>', { 'id': 'input_dept' })),
        $('<p>', { 'html': '文章名稱:' }).append($('<input>', { 'id': 'input_background' })),
        $('<p>', { 'html': '超連結:' }).append($('<input>', { 'id': 'input_urlpath', 'readonly': 'readonly', 'value': '' })).hide(),
